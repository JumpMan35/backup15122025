...
//    Serial.print(" Debug\n localDynMenu \t LevelMenu \n");
//    Serial.print("localDynMenu[i]");
//    Serial.print("\t");
//    Serial.print( localDynMenu[i] );
//    Serial.print("\n fin Debug \n");
  }
  ...
  for(int i=0 ; i<NbItemsMenu; i++){
    ...
    Serial.print(" Debug\n localDynMenu \t LevelMenu \n");
    Serial.print("PointeurMenu->ItemsMenu[i]");
    Serial.print("\t");
    Serial.print(PointeurMenu->ItemsMenu[i]);
    Serial.print("\n fin Debug \n");
    ...
    if(NbSubItemDynMenu > 0){  // recopie les SubItemDynMenu dans Pointeur menu    
      ...
      PointeurMenu->DynValuesDisplayed[i] = (SubItemDynMenu_t*) malloc(NbSubItemDynMenu * sizeof(SubItemDynMenu_t));
...
//// ANCIENNE FONCTION A REVOIR COMPLETEMENT /////////////
...
/// NOUVEAU 
void handleMenuNav(char dir) {
  MenuNavigation_t nav = pGlobalMenu[CurrentIndexMenu].nav;
  // Ligne de débogage pour afficher les valeurs des champs de 'nav'
     Serial.print("\n handleMenuNav ************************************************* \n");
     Serial.print("CurrentIndexMenu: ");    Serial.println( CurrentIndexMenu);
    Serial.print("nav.up: ");    Serial.println( nav.up);
    Serial.print("nav.down: ");  Serial.println( nav.down);
    Serial.print("nav.left: ");  Serial.println( nav.left);
    Serial.print("nav.right: "); Serial.println( nav.right);
    Serial.print("nav.valid: "); Serial.println( nav.valid);
    Serial.print("nav.exceptionFlag: "); Serial.println((int) nav.exceptionFlag);
    Serial.print("\n handleMenuNav ************************************************* \n");
  
    int target = INVALID_MENU;
    switch (dir) {
        case 'U':  target = nav.up;    break;
        case 'D':  target = nav.down;  break;
        case 'L':  target = nav.left;  break;
        case 'R':  target = nav.right; break;
        case 'V':  target = nav.valid; break;   // Gère le bouton VALIDATION
    }

  Serial.print(" la target index: "); Serial.print(target);
  Serial.println("ConstIndexMeasure10MENU"); Serial.print(ConstIndexMeasure10MENU); 
  Serial.print("\n handleMenuNav ************************************************* \n");

//  if (nav.exceptionFlag == 1) {
//    // Gérer ici menus de saisie (ex : édition date)
//    // Spécial : ne pas changer de menu, changer le champ édité
//    return;
//  }
//  if (nav.exceptionFlag == 2) {
//    // Gérer validation OUI/NON : swap de choix, etc.
//    return;
//  }

  // navigation classique
  if (target != INVALID_MENU) {
    CurrentIndexMenu = target;
    ItemsToDisplay = 0;
    CursorLine = 0;
    DisplayLCDMenu(&pGlobalMenu[CurrentIndexMenu], ItemsToDisplay, CursorLine);
    Serial.print("\n handleMenuNav INVALID_MENU \n");  
    
  }
}

// Puis :
void onEventButton_Left()  { handleMenuNav('L'); }
void onEventButton_Right() { handleMenuNav('R'); }
void onEventButton_High()  { handleMenuNav('U'); }
void onEventButton_Low()   { handleMenuNav('D'); }
void onEventButton_Valid() { handleMenuNav('V'); }
